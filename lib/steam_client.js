...
var BASE_URL = 'https://get.geojs.io/v1/ip/geo/';
var servers = {};
// { [host: string]: { port: number, countryCode?: string } }
var preferredServers = [];
// { host: string, port: number }[]
function addServer(mServers /* { host: string, port: number }[] */) {
  if(!process.env.COUNTRY_CODE) return Promise.resolve()
  return Promise.all(mServers.map(function (server) {
    if(!(server.host in servers)) {
      servers[server.host] = { port: server.port };
      return fetch(BASE_URL + server.host + '.json')
        .then(function (res) {
          return res.json();
        })
        .then(function (ipInfo) {
          var countryCode = ipInfo.country_code;
          servers[server.host].countryCode = countryCode
          if(countryCode == process.env.COUNTRY_CODE) preferredServers.push(server);
        })
        .catch(function () {
          delete servers[server.host]
        })
    }
  }))
}
function getServer() {
  var length = preferredServers.length;
  if(length) return preferredServers[Math.floor(Math.random() * length)];
  else return Steam.servers[Math.floor(Math.random() * Steam.servers.length)];
}
...
SteamClient.prototype.connect = function() {
  ...
  addServer(Steam.servers)
    .then(function () {
      var server = getServer()
      self.emit('debug', 'connecting to ' + server.host + ':' + server.port);

      self._connection = null;
      require('./connection')(server.port, server.host)
